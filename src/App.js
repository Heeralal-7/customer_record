...
const C = {
  ...
  accent2:   "#ff6b6b",   // coral-red  (delete / warning)
  accent3:   "#ffd166",   // golden-yellow (edit / highlight)
  accent4:   "#a78bfa",   // violet (new badge)
  text:      "#e6edf3",
  muted:     "#7d8590",
  tableHead: "#0d1117",
  row1:      "#161b22",
  row2:      "#1a202c",
  rowNew:    "#0d2b24",   // fresh-add tint
};
...
function nextId(list) {
  const max = list.reduce((m, c) => {
    const n = parseInt(c.id.replace("CLT-","")) || 0;
    return n > m ? n : m;
  }, 0);
...
function validate(f) {
  ...
  if (!/^\d{10}$/.test(f.phone))               e.phone = "Enter a valid 10-digit phone";
  if (!f.item.trim())                          e.item  = "Item name is required";
  if (!f.date)                                 e.date  = "Purchase date is required";
  if (!f.rate || isNaN(f.rate) || +f.rate<=0)  e.rate  = "Enter a valid rate (₹)";
  if (!f.qty  || isNaN(f.qty)  || +f.qty <=0)  e.qty   = "Enter a valid quantity";
  return e;
}

// ── tiny helpers ──────────────────────────────────────────
const Pill = ({ children, bg, color }) => (
  <span style={{ background:bg, color, borderRadius:6, padding:"3px 10px", fontSize:12, fontWeight:700, fontFamily:"'DM Mono',monospace", whiteSpace:"nowrap" }}>
    {children}
  </span>
);
...
const Field = ({ label, field, form, setForm, errors, type="text", placeholder="" }) => (
  <div style={{ marginBottom:14 }}>
    <label style={{ display:"block", fontSize:11, fontWeight:700, letterSpacing:1.2, textTransform:"uppercase", color:C.muted, marginBottom:5 }}>{label}</label>
    <input
      type={type}
      value={form[field]}
      ...
      style={{
        ...
        background:"#0d1117", fontSize:14, color:C.text, outline:"none",
...
export default function App() {
  ...
  useEffect(() => { save(customers); }, [customers]);
  ...
  const filtered = useMemo(() => {
    ...
    return customers
      ...
      .sort((a,b) => {
        // NEW entries always float to the very top
        if (a.isNew && !b.isNew) return -1;
        if (!a.isNew && b.isNew) return 1;
        // then apply column sort within each group
        const { key, dir } = sort;
  ...
  const submit = () => {
    ...
    if (editId) {
      ...
      setCustomers(p => [entry, ...p]);   // NEW ENTRY AT TOP
      setPage(1);
  ...
  const SortArrow = ({ col }) => {
    if (sort.key!==col) return <span style={{ opacity:.3, fontSize:9, marginLeft:4 }}>⇅</span>;
    return <span style={{ color:C.accent, fontSize:9, marginLeft:4 }}>{sort.dir==="asc"?"▲":"▼"}</span>;
  };
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'DM Sans',sans-serif", color:C.text }}>
      ...
      <div style={{
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:16 }}>
          <div style={{ display:"flex", alignItems:"center", gap:14 }}>
            <div style={{ background:"linear-gradient(135deg,#00d4aa22,#00d4aa44)", border:`1px solid ${C.accent}44`, borderRadius:14, padding:"10px 14px", fontSize:28 }}>👔</div>
            <div>
              <h1 style={{ margin:0, fontSize:24, fontWeight:800, letterSpacing:.5, background:`linear-gradient(90deg, ${C.accent}, #00b4d8)`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>
              Ashoka Textiles 
              </h1>
              <p style={{ margin:0, fontSize:11, color:C.muted, letterSpacing:2, textTransform:"uppercase", fontFamily:"'DM Mono',monospace" }}>Customer Records Manager</p>
            </div>
          </div>
          ...
          <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
            {[
              { label:"Total Customers", value:customers.length,          color:C.accent,  icon:"👥" },
              { label:"New (This Session)", value:newCount,               color:C.accent4, icon:"🆕" },
              ...
              <div key={s.label} style={{
                ...
                borderRadius:12, padding:"10px 18px", textAlign:"center", minWidth:110
              }}>
                <div style={{ fontSize:18 }}>{s.icon}</div>
                <div style={{ color:s.color, fontWeight:800, fontSize:17, fontFamily:"'DM Mono',monospace" }}>{s.value}</div>
                <div style={{ color:C.muted, fontSize:10, letterSpacing:1, textTransform:"uppercase", marginTop:2 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── TOOLBAR ── */}
      <div style={{ padding:"20px 36px 0", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        ...
        >
          <span style={{ fontSize:18, lineHeight:1 }}>＋</span> Add Customer
        </button>
      </div>

      {/* ── TABLE ── */}
      <div style={{ padding:"16px 36px 48px", overflowX:"auto" }}>
        <div style={{ borderRadius:14, overflow:"hidden", border:`1px solid ${C.border}`, boxShadow:"0 4px 40px rgba(0,0,0,.5)" }}>
              ...
              <tr style={{ background:C.tableHead, borderBottom:`2px solid ${C.accent}44` }}>
                {cols.map(c => (
                  <th key={c.key} onClick={() => doSort(c.key)} style={{
                    ...
                    fontFamily:"'DM Mono',monospace", fontWeight:600, fontSize:11, letterSpacing:1.3, textTransform:"uppercase",
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding:"12px 16px" }}>
                      <span style={{ background:"#1a2a3a", color:"#60a5fa", borderRadius:6, padding:"3px 9px", fontSize:12, fontFamily:"'DM Mono',monospace", fontWeight:600 }}>{c.id}</span>
                      {c.isNew && <span style={{ marginLeft:6, background:"#2a1a3a", color:C.accent4, borderRadius:6, padding:"2px 7px", fontSize:10, fontFamily:"'DM Mono',monospace", fontWeight:700 }}>NEW</span>}
                    </td>
                    {/* Name */}
                    <td style={{ padding:"12px 16px" }}>
                    ...
                    <td style={{ padding:"12px 16px", textAlign:"center" }}>
                      <span style={{ background:"#1a1a3a", color:C.accent4, borderRadius:6, padding:"3px 10px", fontFamily:"'DM Mono',monospace", fontWeight:700, fontSize:13 }}>{c.qty}</span>
                    </td>
                    {/* Total */}
            ...
            {paginated.length>0 && (
              <tfoot>
                <tr style={{ background:"#0a1a12", borderTop:`2px solid ${C.accent}44` }}>
                  <td colSpan={7} style={{ padding:"11px 16px", fontFamily:"'DM Mono',monospace", fontSize:12, color:C.muted, letterSpacing:.5 }}>
                    PAGE TOTAL — {paginated.reduce((s,c)=>s+c.qty,0)} items
                  </td>
                  <td style={{ padding:"11px 16px", fontFamily:"'DM Mono',monospace", fontWeight:800, color:C.accent, fontSize:15 }}>
        ...
        {pages>1 && (
          <div style={{ display:"flex", justifyContent:"center", alignItems:"center", gap:8, marginTop:20, flexWrap:"wrap" }}>
            ...
            <PagBtn label="Next ›" disabled={safePage===pages} onClick={()=>setPage(p=>Math.min(pages,p+1))} />
          </div>
        )}
        <p style={{ textAlign:"center", marginTop:10, color:C.muted, fontSize:13, fontFamily:"'DM Mono',monospace" }}>
      ...
      {modal && (
        <div style={{
          ...
          <div style={{
            background:C.card, borderRadius:18, padding:36, width:"100%", maxWidth:540,
            border:`1px solid ${editId ? C.accent3+"66" : C.accent+"66"}`,
            boxShadow:`0 24px 80px rgba(0,0,0,.7)`, maxHeight:"90vh", overflowY:"auto"
          }}>
            {/* modal header */}
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
              <div>
                <h2 style={{ margin:0, fontSize:20, fontWeight:800, color: editId ? C.accent3 : C.accent }}>
                  {editId ? "✏️ Edit Customer Record" : "➕ Add New Customer"}
                </h2>
                <p style={{ margin:"4px 0 0", fontSize:12, color:C.muted }}>
                  {editId ? "Update the fields below and save" : "Fill in all fields — saved to localStorage automatically"}
                </p>
              </div>
              <button onClick={()=>setModal(false)} style={{
                background:"#ffffff10", border:`1px solid ${C.border}`, borderRadius:8, width:34, height:34,
                cursor:"pointer", fontSize:18, color:C.muted, display:"flex", alignItems:"center", justifyContent:"center"
              }}>×</button>
            </div>
            ...
            {form.rate && form.qty && !isNaN(form.rate) && !isNaN(form.qty) && +form.rate>0 && +form.qty>0 && (
              <div style={{
                ...
                <span style={{ color:C.muted, fontSize:13 }}>💰 Total Amount Preview</span>
            ...
            <div style={{ display:"flex", gap:12 }}>
              <button onClick={()=>setModal(false)} style={{
                ...
                fontFamily:"'DM Sans',sans-serif", fontWeight:700, fontSize:14, transition:"background .15s"
              }}
                onMouseEnter={e=>e.target.style.background="#ffffff0a"}
                onMouseLeave={e=>e.target.style.background="transparent"}
              >Cancel</button>
              <button onClick={submit} style={{
                flex:2, padding:12, borderRadius:10, border:"none",
                background: editId ? `linear-gradient(135deg, ${C.accent3}, #ffb830)` : `linear-gradient(135deg, ${C.accent}, #00b4a0)`,
                color:"#0d1117", cursor:"pointer", fontFamily:"'DM Sans',sans-serif",
                fontWeight:800, fontSize:15, boxShadow: editId ? `0 4px 18px ${C.accent3}44` : `0 4px 18px ${C.accent}44`,
                transition:"transform .15s, box-shadow .15s"
              }}
                onMouseEnter={e=>{ e.target.style.transform="translateY(-1px)"; }}
                onMouseLeave={e=>{ e.target.style.transform=""; }}
              >
                {editId ? "💾 Save Changes" : "✅ Add & Save Customer"}
              </button>
            </div>
          </div>
        </div>
      ...
      {delId && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.8)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div style={{ background:C.card, borderRadius:16, padding:32, maxWidth:380, width:"100%", textAlign:"center", border:`1px solid ${C.accent2}44`, boxShadow:"0 20px 60px rgba(0,0,0,.7)" }}>
            <div style={{ fontSize:44, marginBottom:12 }}>⚠️</div>
            <h3 style={{ margin:"0 0 10px", color:C.accent2, fontWeight:800 }}>Delete Record?</h3>
            <p style={{ color:C.muted, marginBottom:24, fontSize:14, lineHeight:1.6 }}>
              You're about to permanently remove customer <span style={{ color:C.text, fontFamily:"'DM Mono',monospace", fontWeight:600 }}>{delId}</span>. This will also clear it from localStorage.
            </p>
            <div style={{ display:"flex", gap:12 }}>
              <button onClick={()=>setDelId(null)} style={{ flex:1, padding:12, borderRadius:10, border:`1px solid ${C.border}`, background:"transparent", color:C.muted, cursor:"pointer", fontWeight:700, fontFamily:"'DM Sans',sans-serif" }}>Cancel</button>
              <button onClick={confirmDel} style={{ flex:1, padding:12, borderRadius:10, border:"none", background:C.accent2, color:"#fff", cursor:"pointer", fontWeight:800, fontFamily:"'DM Sans',sans-serif", boxShadow:`0 4px 14px ${C.accent2}55` }}>Yes, Delete</button>
            </div>
          </div>
        </div>
      )}
